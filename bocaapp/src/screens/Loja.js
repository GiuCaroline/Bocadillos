...
import { Sparkle, Plus, Minus } from 'phosphor-react-native';
...
export function Loja() {
  ...
  return (
    <View className="flex-1 bg-background">
          ...
          <View className="flex-row flex-wrap justify-between mt-[3%]">
            {produtosFiltrados.map((item) => (
              <View
                key={item.id}
                style={styles.sombra}
                className="bg-branco rounded-[15px] p-2 py-3 items-center w-[48%] mb-4"
              >
                {/* Altura fixa de 120px para a imagem — evita espaço em branco */}
                <Image
                ...
                <Text className="font-montserrat-light text-[10px] text-cinza">{item.descript}</Text>
      ...
      <Nav active="Loja" onChange={(r) => navigation.navigate(r)} />
    </View>
  );
}
