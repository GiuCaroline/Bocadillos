...
export function Passo3({ moveStep, customCart, setCustomCart, quantity, setQuantity }) {
    ...
    function selectDetail(detailName) {
        ...
        if (selectedDetails.includes(detailName)) {
            ...
            // Caso contrário, defina-o como a única seleção
            selectedDetails = [detailName];
        }
                            ...
                            <View className='flex-row flex-wrap justify-center gap-4 w-full'>
                                {detailsList.map((detail) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            key={detail}
                                            onPress={() => selectDetail(detail)}
                                            style={styles.shadow}
                                            className={`w-[48%] h-20 mb-4 justify-center items-center rounded-[30px] ${isSelected ? 'bg-marron' : 'bg-branco'}`}
                                        >
                                            <Text numberOfLines={2} className={`font-montserrat-regular text-center text-[14px] ${isSelected ? 'text-branco' : 'text-cinza'}`}>
                                                {detail}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                ...
                <View className="bg-background w-full flex-1 px-4">
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                        ...
                        <View className={`${customCart.description ? 'flex' : 'hidden'} mt-5`}>
                            ...
                            <Text className="text-base text-cinza font-montserrat-light text-left mt-1">
