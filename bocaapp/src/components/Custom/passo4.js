...
export function Passo4({ moveStep, customCart, setCustomCart, quantity, setQuantity }) {
    ...
    function selectpackage_size(size, sizePrice) {
        setCustomCart({
            ...customCart,
            package: { package_size: size, package_price: sizePrice, package_name: `${size <= 10 ? 'Pequeno' : size <= 50 ? 'Médio' : 'Grande'}` }
        });
    }

    function setDesc(desc) {
        setCustomCart({ ...customCart, description: desc });
    }
    ...
    const finalizarCriacao = async () => {
        try {
            // Salva sem o require() — o Carrinho recupera a imagem pelo nome via BASE_IMAGES
                        ...
                        <View className='w-full'>
                            <View className="w-full mt-[10%]">
                                ...
                                <View className='flex-row justify-center flex-wrap gap-4 w-full'>
                                    <TouchableOpacity onPress={() => selectpackage_size(10, 0)} style={styles.shadow}
                                        className={`w-[48%] h-20 mb-4 justify-center items-center rounded-[30px] ${customCart.package?.package_size === 10 ? 'bg-marron' : 'bg-branco'}`}>
                                        <Text className={`font-montserrat-semibold text-[14px] text-center ${customCart.package?.package_size === 10 ? 'text-branco' : 'text-cinza'}`}>Pequeno</Text>
                                        <Text className={`font-montserrat text-[12px] ${customCart.package?.package_size === 10 ? 'text-branco' : 'text-cinza'}`}>(10 uni.)</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => selectpackage_size(50, 38)} style={styles.shadow}
                                        className={`w-[48%] h-20 mb-4 justify-center items-center rounded-[30px] ${customCart.package?.package_size === 50 ? 'bg-marron' : 'bg-branco'}`}>
                                        <Text className={`font-montserrat-semibold text-[14px] text-center ${customCart.package?.package_size === 50 ? 'text-branco' : 'text-cinza'}`}>Médio</Text>
                                        <Text className={`font-montserrat text-[12px] ${customCart.package?.package_size === 50 ? 'text-branco' : 'text-cinza'}`}>(50 uni.)</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => selectpackage_size(100, 86)} style={styles.shadow}
                                        ...
                                        <Text className={`font-montserrat-semibold text-[14px] text-center ${customCart.package?.package_size === 100 ? 'text-branco' : 'text-cinza'}`}>Grande</Text>
                                        <Text className={`font-montserrat text-[12px] ${customCart.package?.package_size === 100 ? 'text-branco' : 'text-cinza'}`}>(100 uni.)</Text>
                                    </TouchableOpacity>
                                </View>

                                <Text className="text-[15px] font-montserrat-bold text-cinza text-center mt-8">O que você gostaria de alterar?</Text>
                                <TextInput multiline maxLength={160} value={customCart.description || ''} onChangeText={setDesc}
                                    placeholder="Digite aqui a sua alteração..." placeholderTextColor="#a1a1aa"
                ...
                <View className="bg-background w-full flex-1 px-4">
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                        <View className='flex-row items-center'>
                            ...
                            <View className='ml-[5%] mt-[5%] flex-1'>
                                ...
                                <View className={`${customCart.flavors && customCart.flavors.length > 0 ? 'flex' : 'hidden'}`}>
                                    ...
                                    <Text className="text-sm text-cinza font-montserrat-light text-left mt-[-4%]">{customCart.flavors ? customCart.flavors.join(", ") : ''}</Text>
                                </View>
                                ...
                                <View className={`${customCart.details && customCart.details.length > 0 ? 'flex' : 'hidden'}`}>
                                    ...
                                    <Text className="text-sm text-cinza font-montserrat-light text-left mt-[-4%]">{customCart.details ? customCart.details.join(", ") : ''}</Text>
                                </View>
                            </View>
                        </View>
                        ...
                        <View className={`${customCart.description ? 'flex' : 'hidden'} mt-5`}>
                            ...
                            <Text className="text-base text-cinza font-montserrat-light text-left mt-[-2%]">{customCart.description}</Text>
                        </View>
